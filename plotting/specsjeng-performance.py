...
perf_dir = os.path.join(cur_dir, '..', 'spec-performance-opt', 'specsjeng')
assets_dir = os.path.join(cur_dir, '..', 'assets', 'specsjeng')
...
plt.xlabel("L1D Size")
plt.ylabel("CPI")
plt.title("specsjeng: CPI vs L1D Size")
plt.grid(True)
plt.savefig(os.path.join(assets_dir, "cpi_vs_l1d_size.png"))

# 2. CPI vs L1D associativity
cpi_l1d_assoc = []
for l1d_assoc in l1d_assocs:
    code = f"{base_l1i_size}_{base_l1i_assoc}_{base_l1d_size}_{l1d_assoc}_{base_l2_size}_{base_l2_assoc}_{base_line_size}"
    stats_path = os.path.join(perf_dir, code, "stats.txt")
    cpi = get_cpi_from_stats(stats_path)
    cpi_l1d_assoc.append(cpi)

plt.figure()
plt.plot(l1d_assocs, cpi_l1d_assoc, marker='o')
plt.xlabel("L1D Associativity")
plt.ylabel("CPI")
plt.title("specsjeng: CPI vs L1D Associativity")
plt.grid(True)
plt.savefig(os.path.join(assets_dir, "cpi_vs_l1d_assoc.png"))
...
plt.xlabel("L2 Size")
plt.ylabel("CPI")
plt.title("specsjeng: CPI vs L2 Size")
plt.grid(True)
plt.savefig(os.path.join(assets_dir, "cpi_vs_l2_size.png"))

# 4. CPI vs L2 associativity
cpi_l2_assoc = []
for l2_assoc in l2_assocs:
    code = f"{base_l1i_size}_{base_l1i_assoc}_{base_l1d_size}_{base_l1d_assoc}_{base_l2_size}_{l2_assoc}_{base_line_size}"
    stats_path = os.path.join(perf_dir, code, "stats.txt")
    cpi = get_cpi_from_stats(stats_path)
    cpi_l2_assoc.append(cpi)

plt.figure()
plt.plot(l2_assocs, cpi_l2_assoc, marker='o')
plt.xlabel("L2 Associativity")
plt.ylabel("CPI")
plt.title("specsjeng: CPI vs L2 Associativity")
plt.grid(True)
plt.savefig(os.path.join(assets_dir, "cpi_vs_l2_assoc.png"))
...
plt.ylabel("CPI")
plt.title("specsjeng: CPI vs Cache Line Size")
plt.grid(True)
